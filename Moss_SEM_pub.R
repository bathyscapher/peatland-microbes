...
moss <-
'MossHabitat ~ Altitude + Distance + CanopyCover
ev.p ~ Altitude + Distance + CanopyCover
ev.e ~ Altitude + Distance + CanopyCover

mb.moss.prok ~ Distance + Altitude + ev.p + MossHabitat + CanopyCover +
  mb.moss.euk #+ Moss_g
mb.moss.euk ~  Distance + Altitude + ev.e + MossHabitat + CanopyCover +
  mb.moss.prok #+ Moss_g
...
#
# modificationindices(fit.moss.up, minimum.value = 3, op = "~")


fit.moss.b <- bsem(moss, data = mossMeta, n.chains = 4,
                   burnin = 10000, sample = 60000,
                   bcontrol = list(cores = 8),
                   control = list(max_treedepth = 10, list(adapt_delta = 0.8)))
...
semPaths(fit.moss.up, what = "est", whatLabels = "est", residuals = FALSE,
...
library("lavaanPlot")
labels <- list(eve = "Habitat\nquality", evp = "Habitat\nquality",
               Moss_g = "Sampling\nintensity",
               mbmossprok = "Prokaryotes",
               mbmosseuk = "Eukaryotes")

lavaanPlot(model = fit.moss, labels = labels, #sig = 0.05, stars = TRUE,
...
mb.moss.prok ~ Distance + Altitude + CanopyCover + MossHabitat +
  mb.moss.euk + ev.p
mb.moss.euk ~ Distance + Altitude + CanopyCover + MossHabitat +
  mb.moss.prok + ev.e
...
## Update model
fit.moss.v2.up <- update(fit.moss.v2, add = "ev.p ~~ ev.e")
summary(fit.moss.v2.up, fit.measures = TRUE, rsq = TRUE)

modificationindices(fit.moss.v2.up, minimum.value = 3, op = "~")
